...
/*------------------------------------------------------------------------------*/
 ...
 document.querySelector('.query_form_1').addEventListener('submit', async function(event) {
    ...
    if (!name || !email || !propertyAddress || !phone || !serviceType || !preferredContact || !message) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const nextUrl = formData.get('_next') || 'thank-you.html';
            window.location.href = nextUrl; // Redirect to _next URL
        } else {
...
document.querySelector('.query_form_1').addEventListener('submit', function(event) {
    const consent = document.querySelector('#comment-cookies-consent').checked;
    if (consent) {
...
window.addEventListener('load', function() {
    ...
    if (savedEmail) document.querySelector('input[name="email"]').value = savedEmail;
});
